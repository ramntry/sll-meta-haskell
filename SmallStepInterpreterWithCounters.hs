module SmallStepInterpreterWithCounters (int) where
...
intStep p (FCall name args, counter) = (body // zip vs args, counter + 1) where
  (FDef _ vs body) = fDef p name

intStep p (GCall gname (Ctr cname cargs : args), counter) =
  (body // zip (cvs ++ vs) (cargs ++ args), counter + 1) where
