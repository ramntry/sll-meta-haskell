module BigStepInterpreterCallByValue (int) where
...
int p (FCall name args) counter = int p (body // zip vs evaledArgs) (updatedCounter + 1) where
  (FDef _ vs body) = fDef p name
  (evaledArgs, updatedCounter) = listInt p args counter

int p (GCall gname (Ctr cname cargs : args)) counter =
  int p (body // zip (cvs ++ vs) evaledArgs) (updatedCounter + 1) where
    (GDef _ (Pat _ cvs) vs body) = gDef p gname cname
    (evaledArgs, updatedCounter) = listInt p (cargs ++ args) counter
